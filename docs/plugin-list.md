...
添加后可以在“在线插件”中浏览、安装和更新本仓库收录的插件。插件源索引也可以收录其他 GitHub 仓库中的插件：在 `echo-plugins.json` 中为插件填写 `repo` 即可指向独立插件仓库，`path` 为空时表示插件位于该仓库根目录。网络环境不佳时，可在 EchoMusic 设置的“实验性功能”里配置 GitHub 加速地址，插件源索引和插件下载会复用该地址。
